...
def createQueueItem():
    ...
    if data['ok']:
      ...
      newQueueItem = {}
      newQueueItem['longrunningtaskID'] = longrunningtaskID 
...
# Queue user and longrunningtask collection for the IDs saved in queue, flatten return value, only return the three values in project back to frontend
def getQueue():
  try:
...
# Get user who owns longrunningtask from queue item
# If current user == owner -> delete queue item
# Return the longrunningtask ID and user ID which were stored inside the queue item
def deleteQueueItem():
  try:
    ...
    ids['longrunningtaskID'] = itemToDelete['longrunningtaskID']
