...
# Look up longrunningtask details in collection by longrunningtask ID saved in user history array
...
def getUserDetails():
    ...
    data =  mongo.db.users.aggregate(
      [
        ...
        {"$lookup": {"from": "longrunningtasks", "foreignField": "_id", "localField": "history", "as": "submittedJobs"}},
...
# Add new ID to set of longrunningtask IDs
def updateUserHistory():
  try:
    current_user = get_jwt_identity()
    data = request.get_json()
    user = mongo.db.users.find_one({'email': current_user['email']})
    longrunningtaskID = ObjectId(data['longrunningtaskID'])
    user = mongo.db.users.find_and_modify({'email': current_user['email']}, {"$addToSet": {"history": longrunningtaskID}})

    returnValue = {}
    returnValue['longrunningtaskID'] = data['longrunningtaskID']
...
# Find a user by ID, find a longrunningtask in the user's history by ID, remove the longrunningtask from the history, 
# Return the ID of the longrunningtask
def removeFromUserHistory():
  try:
    data = request.get_json()
    userID = data['userID']['$oid']
    longrunningtaskID = data['longrunningtaskID']['$oid']
    user = mongo.db.users.find_and_modify({'_id': ObjectId(userID)}, {"$pull": {"history": ObjectId(longrunningtaskID)}})
    returnValue = {}
    returnValue['longrunningtaskID'] = longrunningtaskID
    return jsonify({'ok': True, 'data': returnValue}), 200
  except:
     return jsonify({'ok': False, 'message': 'Bad request parameters: {}'.format(data['message'])}), 400
