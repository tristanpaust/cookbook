...
class Queue extends React.Component {
  ...
  // The popup is for the user to confirm before a LongRuningTask is deleted
  ...
  // Return ID of user who submitted the item (must obviously be the one whose deleting) as well as the ID of the LongRuningTask
  // Pull the LongRuningTask ID from the users history
  // Return the ID of the LongRuningTask
  // Remove LongRuningTask from collection with ID
  // Return link to file associated with LongRuningTask
  ...
  deleteQueueItem(event) {
    ...
    this.apiClient.deleteQueueItem({'queueID': queueID}).then((data) => {
      this.apiClient.removeFromUserHistory(data).then((res) => {
        ...
        }).catch((err) => { console.log('Error deleting the LongRuningTask ', err) })
      }).catch((err) => { console.log('Error deleting the LongRuningTask from the users history ', err) })
  ...
  // Print creator name, LongRuningTask name, when the LongRuningTask started and for how long it has been running 
  ...
  createItems(item) {
    ...
    let startTime = new Date(item.LongRuningTask.timeStarted.$date);
    ...
    return (
      <tr key={item._id.$oid} id={item._id.$oid}>
        <td> {item.LongRuningTask.LongRuningTaskTitle} </td> 
